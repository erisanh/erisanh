...
- **Dependencies**: Use `uv add` (never hand-edit the `dependencies` field in `pyproject.toml`).
- **API Design**: Follow [PEP 8](https://peps.python.org/pep-0008/) and use [type hints (PEP 484)](https://peps.python.org/pep-0484/).
- **Ad-hoc Debugging**: Put temp scripts in `tmp/`, run with `uv run tmp/debug_task.py`, and delete once resolved.
